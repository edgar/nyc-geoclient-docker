...
RUN apt-get update \
    && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends wget unzip git openjdk-8-jdk maven build-essential
...
RUN cp /app/geoclient/geoclient-service/build/libs/geoclient-service-2.0.0-rc.1.war $CATALINA_HOME/webapps/ROOT.war
